...
The frontend uses `src/mock/processed_metrics.json`, generated by the backend pipeline. This keeps the UI focused on presentation and resilience states while the backend remains the source of truth for calculations. In a production version, the UI would fetch a materialized metrics endpoint or subscribe to live updates.
